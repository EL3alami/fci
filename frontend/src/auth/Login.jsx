...
import { Eye, EyeOff, User, BookOpen, Shield, Loader2 } from 'lucide-react';
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.role && data.role !== selectedRole.id) {
        // Warning: Backend role might not match frontend ID exactly, handle with care.
        // For now assuming strict match.
        if (data.role.toLowerCase() !== selectedRole.id) {
          throw new Error(`This account is not registered as a ${selectedRole.name}`);
        }
      }
      ...
      {step === 'role' ? (
        ...
        <div className="w-full max-w-lg bg-white/70 backdrop-blur-lg border border-white/40 rounded-3xl shadow-2xl mx-auto px-7 py-8 sm:px-14 sm:py-12 flex flex-col items-center relative" style={{ boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.18)' }}>
          ...
          <div className="w-full px-2 sm:px-4">
            ...
            {error && (
              <div className="w-full mb-4 p-3 text-sm text-red-600 bg-red-100 rounded-xl border border-red-200 text-center">
                {error}
              </div>
            )}
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-5 items-center">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-72 max-w-full py-3 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-extrabold rounded-xl shadow-lg transition-all text-lg mt-2 tracking-wide mx-auto flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed"
              >
